  ...
  handleError(error: Error | HttpErrorResponse) {
    ...
    if (error instanceof HttpErrorResponse) {
      ...
      if (!navigator.onLine) {
        ...
        console.log('tssg.ErrorHandler.handleError: offline Error' + error.name + ': ' + error.message);
      } else {
        // Handle Http Error (error.status === 403, 404...)
        console.log('tssg.ErrorHandler.handleError: Http Error' + error.name + ': ' + error.message);
      }
    } else {
